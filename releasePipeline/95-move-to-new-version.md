...
2. Upgrade [Galasa](https://github.com/galasa-dev/galasa)
    ...
    **Note:** Once the Galasa mono repo's build finishes, this will trigger the `update-prod1` Tekton pipeline, which will then trigger the `run-tests` Tekton pipeline. The `run-tests` will fail as the CPS properties have not yet been upgraded to the new development version (unless you have already done it with galasactl) - this is okay.
