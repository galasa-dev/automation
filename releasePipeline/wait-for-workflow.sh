...
#
# Usage:
#   Mode 1 (by run ID): wait-for-workflow.sh --repo <owner/repo> --run-id <id> --name <display-name> [--sleep <seconds>] [--max-iterations <count>]
#   Mode 2 (by search):  wait-for-workflow.sh --repo <owner/repo> --workflow <file> --branch <name> --start-time <timestamp> --name <display-name> [--sleep <seconds>] [--max-iterations <count>]
#
#-----------------------------------------------------------------------------------------     
...
function usage {
    info "Usage:"
    info "  Mode 1 (by run ID):  wait-for-workflow.sh --repo <owner/repo> --run-id <id> --name <display-name> [--sleep <seconds>] [--max-iterations <count>]"
    info "  Mode 2 (by search):  wait-for-workflow.sh --repo <owner/repo> --workflow <file> --branch <name> --start-time <timestamp> --name <display-name> [--sleep <seconds>] [--max-iterations <count>]"
    info ""
    info "Required parameters (all modes):"
    ...
    info ""
    info "Mode 1 - Wait for specific run ID (recommended when you have the run ID):"
    info "  --run-id            Workflow run ID to monitor"
    info ""
    info "Mode 2 - Search for workflow run (when you don't have the run ID):"
...
if [[ -z "$repo" || -z "$workflow_name" ]]; then
    error "Missing required parameters: --repo and --name are required"
    usage
    exit 1
fi
...
if [[ -n "$run_id" ]]; then
    # Mode 1: Direct run ID monitoring
    if [[ -n "$workflow_file" || -n "$branch" || -n "$start_time" ]]; then
        warn "When using --run-id, parameters --workflow, --branch, and --start-time are ignored"
    fi
elif [[ -z "$workflow_file" || -z "$branch" || -z "$start_time" ]]; then
    # Mode 2: Search mode requires all search parameters
    error "When not using --run-id, you must provide --workflow, --branch, and --start-time"
    usage
    exit 1
fi
...
if [[ -n "$run_id" ]]; then
    #-----------------------------------------------------------------------------------------
    # Mode 1: Monitor specific run ID
    #-----------------------------------------------------------------------------------------
    info "Monitoring workflow run ID: ${run_id}"
    info "Repository: ${repo}"
    info "View at: https://github.com/${repo}/actions/runs/${run_id}"
    
    while [[ $counter -lt $max_iterations ]]; do
        info "Checking ${workflow_name} status... (attempt $((counter+1))/$max_iterations)"
        
        # Get the status of the specific run
        run_info=$(gh run view "${run_id}" --repo "${repo}" --json conclusion,status,createdAt 2>&1)
        
        # Check if the command succeeded
        if [[ $? -ne 0 ]]; then
            warn "Failed to query workflow run: ${run_info}"
        ...
        # Validate JSON data
        if ! echo "$run_info" | jq empty 2>/dev/null; then
        ...
        if [[ "$status" == "success" ]]; then
            ...
            info "${workflow_name} status: ${workflow_status}"
        else
    ...
    #-----------------------------------------------------------------------------------------
    # Mode 2: Search for workflow run by branch and time
    #-----------------------------------------------------------------------------------------
    ...
    while [[ $counter -lt $max_iterations ]]; do
        ...
        found_run_id=$(echo "$run_data" | jq -r --arg start_time "$start_time" '.[] | select(.createdAt > $start_time) | .databaseId' 2>/dev/null | head -1)
        
        if [[ -n "$found_run_id" ]]; then
            info "Found ${workflow_name} workflow run: ${found_run_id}"
            
            # Get the full status of this specific run
            run_info=$(echo "$run_data" | jq -r --arg run_id "$found_run_id" '.[] | select(.databaseId == ($run_id | tonumber))')
            status=$(echo "$run_info" | jq -r '.conclusion')
            created_at=$(echo "$run_info" | jq -r '.createdAt')
            ...
            if [[ "$status" == "success" ]]; then
                success "${workflow_name} completed successfully."
                info "View at: https://github.com/${repo}/actions/runs/${found_run_id}"
                exit 0
            elif [[ "$status" == "failure" || "$status" == "cancelled" ]]; then
                error "${workflow_name} failed or was cancelled."
                error "Check https://github.com/${repo}/actions/runs/${found_run_id}"
                exit 1
            elif [[ "$status" == "null" || -z "$status" ]]; then
