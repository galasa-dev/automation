...
function build_cli_to_isolated {

    yaml_file="build_cli_to_isolated.yaml"
...
metadata:
  generateName: complete-build-part2-
  annotations:
...
# 
# 
# Start Part 2 of the Complete Build at the CLI pipeline
# 
# 
  pipelineRef:
    name: branch-cli
...
EOF
    ...
    # pipelinerun.tekton.dev/complete-build-part2-8cbj8 created
    rc=$?
    if [[ "${rc}" != "0" ]]; then
    ...
    success "Build for CLI kicked off - this should trigger a chain of builds up to and including Isolated. (CLI > Eclipse > Isolated)"
    ...
    note "All of the components of Galasa have now been built."
}


ask_user_for_release_type
set_kubernetes_context
build_cli_to_isolated